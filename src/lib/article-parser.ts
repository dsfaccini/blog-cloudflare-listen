...
/**
 * Extract plain text from article content for TTS generation with improved pause handling
 */
export function extractTextForAudio(content: ArticleContent): string {
    const textParts: string[] = [];

    // Add title with hyphen pause
    ...
    for (const block of content.blocks) {
        switch (block.type) {
            case BlockType.HEADING:
                if (block.content) {
                    const headingText = inlineElementsToText(block.content);
            ...
            case BlockType.LIST:
                if (block.items) {
                    // Add hyphen before list
                    textParts.push(' — ');
                    for (const item of block.items) {
                        // Add hyphen after each list item
...
/**
 * Extract plain text from article content for TTS generation with hyphen pauses
 */
export function extractTextForAudioWithHyphenPauses(content: ArticleContent): string {
    const textParts: string[] = [];
