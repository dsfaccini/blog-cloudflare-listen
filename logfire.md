...
async function generateAudioChunk(chunkIndex: number, text: string) {
  return await logfire.span(
    ...
    async (span) => {
      try {
        span.info("Starting audio generation for chunk {chunk_index}", { chunk_index: chunkIndex });
...
```typescript
export async function storeAudioChunk(bucket: R2Bucket, slug: string, chunkIndex: number, audioBuffer: ArrayBuffer): Promise<void> {
  return await logfire.span(
    "store_audio_chunk",
    { 
      slug,
...
```typescript
// OLD
console.log(`Audio generation result: ${result.availableChunks.length}/${result.totalChunks} chunks complete`);
